...
beforeEach(function () {
    config([
        // 'external-api.apis.test-api.repository' => RedisExternalApiTokenRepository::class,
        'external-api.apis.test-api.repository' => 'redis',
    ...
    $this->repository = fn() => app(ExternalApiTokens::class)->repository('test-api');
...
it('saves token with expires_at timestamp', function () {
    ...
    expect($result)->toBeArray()
        ->and($result)->toHaveKey('token_data')
        ->and($result)->toHaveKey('valid_until')
        ->and($result['token_data'])->toBe($tokenData);
});

it('saves token with expires_in seconds and sets TTL', function () {
    $expiresIn = 3600;
    $tokenData = [
        'access_token' => 'test_access_token',
    ...
    // Check that TTL was set in Redis
    $key = cache_key('external_api_tokens')
        ->addParam('type', get_class($this->user))
        ->addParam('id', $this->user->getAuthIdentifier())
        ->addParam('api', 'test-api')
        ->toString();

    $ttl = Redis::ttl($key);
    expect($ttl)->toBeGreaterThan(0)
        ->and($ttl)->toBeLessThanOrEqual($expiresIn);
});

it('encrypts token data before storing in Redis', function () {
    $tokenData = [
        'access_token' => 'test_access_token',
        'expires_in' => 3600,
    ];
    ...
    // Verify it's encrypted by checking it's not the same as JSON
    expect($encrypted)->not->toBe(json_encode($tokenData));

    // Verify it can be decrypted
    $decrypted = Crypt::decryptString($encrypted);
    $data = json_decode($decrypted, true);
    expect($data['token_data'])->toBe($tokenData);
});

it('throws exception when no expiration mapping configured', function () {
    $repository = new RedisExternalApiTokenRepository('test-api', [
        'access_token' => 'access_token',
    ]);
...
})->throws(RuntimeException::class, 'Error resolving the token.');
...
it('invalidates existing token before saving new one', function () {
    ...
    expect($current['access_token'])->toBe('second_token');
});
...
it('throws exception when access token not found in token data', function () {
    $tokenData = [
        'other_field' => 'value',
        'expires_in' => 3600,
    ];

    // Manually save without access_token to bypass save validation
    $key = cache_key('external_api_tokens')
        ->addParam('type', get_class($this->user))
        ->addParam('id', $this->user->getAuthIdentifier())
        ->addParam('api', 'test-api')
    ...
    $validUntil = now()->addSeconds(3600);
    $data = [
    ...
    (repo())->accessToken($this->user);
})->throws(RuntimeException::class, 'No access token found in token data');
...
it('returns null for refresh token when no token exists', function () {
    $refreshToken = (repo())->refreshToken($this->user);
...
it('returns null for refresh token valid until when no token exists', function () {
    $validUntil = (repo())->refreshTokenValidUntil($this->user);
...
it('handles multiple APIs for same user', function () {
    config([
        'external-api.apis.another-api.repository' => RedisExternalApiTokenRepository::class,
...
it('caches retrieved token data in tokenData property', function () {
    $tokenData = [
        'access_token' => 'test_token',
        'expires_in' => 3600,
    ];
...
it('generates correct Redis key for token storage', function () {
    $tokenData = [
        'access_token' => 'test_token',
        'expires_in' => 3600,
    ];
    ...
    $expectedKey = cache_key('external_api_tokens')
        ->addParam('type', get_class($this->user))
        ->addParam('id', $this->user->getAuthIdentifier())
        ->addParam('api', 'test-api')
        ->toString();
...
it('handles token data with Carbon instance in expires_at', function () {
    $expiresAt = now()->addHour();
    $tokenData = [
        'access_token' => 'test_token',
        'expires_at' => $expiresAt, // Pass Carbon instance directly
    ];

    $result = (repo())->save($this->user, $tokenData);

    expect($result)->toBeArray()
        ->and($result['valid_until'])->toBeString();
});

it('sets token without TTL when valid_until is null', function () {
    // This would require a custom scenario where parseValidUntil returns null
    // which shouldn't happen with current validation, but test the code path
    $repository = new class ('test-api', ['access_token' => 'access_token']) extends RedisExternalApiTokenRepository {
    ...
    $tokenData = ['access_token' => 'test_token'];
    $result = $repository->save($this->user, $tokenData);
